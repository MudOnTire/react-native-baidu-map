...
+ (void)setVisualRange:(BaiduMapView *)mapView pointArray:(NSArray *)points
{
    if (points.count == 0) {
        ...
        [mapView setRegion:BMKCoordinateRegionMake(CLLocationCoordinate2DMake((latMax-latMin)/2.0 + latMin, (lngMax-lngMin)/2.0 + lngMin), BMKCoordinateSpanMake(latMax-latMin + 0.0004, lngMax - lngMin + 0.0004)) animated:YES];
    }
}
