 ...
 *
 * @author hzx
 * created at 2018/4/2 17:47
 */
public class ChooseLocationActivity extends Activity implements AdapterView.OnItemClickListener, OnGetGeoCoderResultListener, BaiduMap.OnMapStatusChangeListener, View.OnClickListener {
    ...
    public void onClick(View v) {
        int i = v.getId();
        if (i == R.id.img_location_back_origin) {
            ...
            // Intent search_intent = new Intent(ChooseLocationActivity.this, SearchPositionActivity.class);
            // startActivityForResult(search_intent, REQUEST_CODE);

        } else if (i == R.id.tv_send) {
            Intent intent = new Intent();
            intent.putExtra("localAddress", mLocationAddress);
            intent.putExtra("localName", mLocationName);
            intent.putExtra("localLatitude", mLatLng.latitude + "");
            intent.putExtra("localLongitude", mLatLng.longitude + "");
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position,
        ...
        // 获取位置名称
        mLocationName = info.name;
        //获得详细地址
        mLocationAddress = info.address;
        mLatLng = info.location;
        // Log.e("ChooseLocationActivity", "address=" + mLocationAddress+" name="+mLocationName);
        // 动画跳转
    ...
    public void onGetReverseGeoCodeResult(ReverseGeoCodeResult result) {
        ...
        mLocationAddress = result.getAddress();
        mLocationName = result.getAddress();
        mLoactionLatLng = result.getLocation();
        Log.e("ChooseLocationActivity", "address=" + result.getAddress());
        datas.clear();
        ...
        //在选择位置列表如果不选择，则默认选择第一条数据
        mLatLng = datas.get(0).location;
        tv_send.setBackgroundResource(R.drawable.actionbar_tv_selector);
        tv_send.setEnabled(true);
        Log.e("TAG", "datasSIze=" + datas.size());
        locatorAdapter.notifyDataSetChanged();
